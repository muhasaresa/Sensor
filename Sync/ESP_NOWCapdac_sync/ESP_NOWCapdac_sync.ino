...
void syncCallback(const esp_now_recv_info *recv_info, const uint8_t *incomingData, int len) {
    ...
    else
    {
      ...
      //Serial.println(syncData.sync_time);
...
void Task1(void *pvParameters) {
  ...
  while (1) {
    ...
    //Serial.print(", ");
    vTaskDelay(pdMS_TO_TICKS(10)); // Wait for 7 ms
    ...
    offset += snprintf(buffer+offset,sizeof(buffer),"U%07d\n",previousMillis); // second sensors tx with U
...
void setup() {
  ...
  // accel.setRange(ADXL345_RANGE_2_G);
    ...
    //esp_now_register_send_cb(onDataSent);
    esp_now_register_recv_cb(syncCallback); // function to receive Sync data from server
    // Register peer
