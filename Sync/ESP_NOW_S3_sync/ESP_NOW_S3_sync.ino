...
typedef struct {
    uint32_t sync_time;  // Received master time
} sync_packet_t;
...
uint32_t syncSignal = 0;  // Time offset correction
...
void syncCallback(const esp_now_recv_info *recv_info, const uint8_t *incomingData, int len) {
    ...
    // Calculate time offset (Master time - Local time)
    else
    {
      //USBSerial.println(previousMillis);
      //USBSerial.println(syncData.sync_time);
      previousMillis = syncData.sync_time;
      syncSignal = 1;}
...
void Task1(void *pvParameters) {
  ...
  while (1) {
    ...
    if (! FDC.readMeasurement(MEASURMENT, value))
    {
      ...
      if (syncSignal == 1){ // to check synchronisation
        capacitance = 25000;
        syncSignal = 0;
      }
...
// CORRECTED TIMESTAMP FORMAT to 8 digits (lu for unsigned long)
offset += snprintf(buffer+offset,sizeof(buffer),"T%08lu\n",previousMillis);
...
// put your setup code here, to run once:
 ...
 // Initialize the QMI8658
  ...
  qmi.dumpCtrlRegister();
    ...
    esp_now_register_recv_cb(syncCallback);
    // Register peer
   ...
   // Create Task 3 (Lowest priority)
    ...
    USBSerial.println("ESP32 MAC Address: ");
